"""
Project 02, Sub-Stage 2a -- isolated panel verification.

83x100x2mm panel, pure in-plane axial compression, base fixed,
uniform pressure 0.1643 MPa applied at the free top face (z=100).

Analytical reference (uniaxial membrane, NOT plate bending -- no
transverse load applied): sigma = P/A, delta = PL/(AE)

Plate-buckling closed-form check included (b/t=41.5, high slenderness,
k=4 simply-supported long edges assumed). No eigenvalue buckling FEA.
...
W = 83.0       # x, mm (panel length in the assembled frame)
T = 2.0        # y, mm (thickness)
L = 100.0      # z, mm (height)

E_MOD = 68900.0    # Al 6061-T6, MPa
NU = 0.33
RHO = 2700e-9

PRESSURE = 0.1643  # MPa, from the equal-stress load-sharing calc
AREA = W * T
FORCE = PRESSURE * AREA

MESH_H = 1.5   # mm, safely under the 2mm thickness (>2 elements through)
TOL = 1e-3

OUT_DIR = "mesh/panel_2a_study"
os.makedirs(OUT_DIR, exist_ok=True)
INP_OUT = "simulation/stage2a_panel_solve.inp"
os.makedirs(os.path.dirname(INP_OUT), exist_ok=True)


def main():
    print("=" * 70)
    print("SUB-STAGE 2a: ISOLATED PANEL")
    print("=" * 70)
    print(f"Geometry: {W}x{T}x{L}mm, Area={AREA}mm^2")
    ...
    K_BUCKLE = 4.0
    sigma_cr = K_BUCKLE * (math.pi ** 2) * E_MOD / (12 * (1 - NU ** 2)) * (T / W) ** 2
    P_cr = sigma_cr * AREA
    print(f"Plate buckling (k={K_BUCKLE}): sigma_cr={sigma_cr:.4f} MPa, P_cr={P_cr:.2f} N")
    print(f"Applied/critical stress ratio = {sigma_theory / sigma_cr:.4f}")

    print("\nBuilding geometry...")
    box = Box(Pnt(-W / 2, -T / 2, 0.0), Pnt(W / 2, T / 2, L))
    geo = OCCGeometry(box)
    ...
    with open(INP_OUT, "w") as f:
        f.write("*HEADING\n")
        f.write("Project 02, Sub-Stage 2a -- isolated panel, axial compression\n")
        ...
        f.write("*ELEMENT, TYPE=C3D10, ELSET=EPANEL\n")
        ...
        f.write(f"{E_MOD}, {NU}\n*DENSITY\n{RHO}\n")
        f.write("*SOLID SECTION, ELSET=EPANEL, MATERIAL=AL6061T6\n")
