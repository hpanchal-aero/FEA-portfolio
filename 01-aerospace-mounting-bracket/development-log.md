...
1. `stage6_lhs06_vm_iso.png`, `stage6_lhs06_vm_top.png` — selected
   design (lhs_06, L2 mesh) von Mises contour; the top view clearly
   shows the four-lobe hole-edge concentration pattern, consistent
   with the mechanism identified in Stage 5
2. `stage6_lhs06_vm_front.png`, `stage6_lhs06_vm_back.png` —
   orthogonal elevation views (front is low-information, as in
   Stage 5 — the plain flange face away from the hole carries little
   stress)
3. `stage6_tradespace.png` — mass vs. stress and mass vs.
   displacement, all 25 studied designs (14 fixed-scheme circles +
   11 high-t coarse triangles), feasibility color-coded, selected
   design starred. Visually confirms both headline findings: zero
   feasible fixed-scheme designs (all circles red), and the high-t
   region containing the only feasible points (5 green triangles)
...
- [x] Final documentation and figures
