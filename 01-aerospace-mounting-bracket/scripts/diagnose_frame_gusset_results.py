"""
Stage 4b (right-angle frame WITH gusset) - result extraction:
equilibrium, tip displacement, and mid-span stress at the same
stations used in Stage 4a, for direct comparison. Reuses the exact
validated parsing logic from diagnose_frame_results.py.

Note: the corner (s=60) comparison from 4a is not meaningful here in
the same way, since the old reentrant-corner surface feature no
longer exists (confirmed during geometry construction) - but stress
is still extracted near the hypotenuse region separately, below.
"""

import numpy as np

FRD_FILE = "analysis_frame_gusset.frd"
NSET_FILE = "nsets_frame_gusset.inp"

F = 235.44
L = 60.0

TARGETS_B_MAG = {0: 0.0, 15: 33.1088, 30: 66.2175, 45: 99.3262, 60: 132.4350}
TARGET_A_CONCAVE = -133.9065
TARGET_A_CONVEX = 130.9635
...
print(f"4a (no gusset) fine-level result: -4.2180 mm")
print(f"Reduction from gusset: {100*(mean_ux - (-4.2180))/(-4.2180):+.2f}%")
    ...
    for face_x, face_name, sign in [(58.0, "concave", -1), (62.0, "convex", +1)]:
        ...
        for nid, (x, y, z) in coords.items():
            if abs(z - z_target) < 0.75 and abs(y - 20.0) < 1.5 and abs(x - face_x) < 0.5:
                if nid in stress:
                    candidates.append(stress[nid][2])
        if candidates:
...
convex = []
for nid, (x, y, z) in coords.items():
    if abs(x - 30.0) < 0.5 and abs(y - 20.0) < 1.5:
...
if concave:
    ...
    print(f"Concave (z=+2) sigma_xx: {mean_c:.4f} MPa vs 4a target {TARGET_A_CONCAVE:.4f} MPa ({pct_c:+.2f}%), n={len(concave)}")
else:
...
if convex:
    ...
    print(f"Convex (z=-2) sigma_xx: {mean_v:.4f} MPa vs 4a target {TARGET_A_CONVEX:.4f} MPa ({pct_v:+.2f}%), n={len(convex)}")
else:
    print("Convex: NO MATCH")

# ---- Peak stress ANYWHERE in the model (the key new-information
# question for this stage: where is the worst stress now, and how
# does its magnitude compare to 4a's corner singularity reading?) ----
print("\n=== PEAK VON MISES / SIGMA_XX MAGNITUDE, WHOLE MODEL ===")
all_sxx = [(nid, coords[nid], s[0]) for nid, s in stress.items() if nid in coords]
all_sxx.sort(key=lambda t: abs(t[2]), reverse=True)
print("Top 10 by |sigma_xx|:")
for nid, (x, y, z), sxx in all_sxx[:10]:
    print(f"  node {nid} at ({x:.2f},{y:.2f},{z:.2f}): sigma_xx={sxx:.4f} MPa")
